...
async function waitForGapToWiden({ opportunity, getPrices }) {
  const initialGap = opportunity.gap
  ...
  while (true) {
    ...
    for (const ticket of tickets) {
      try {
        ...
        if (
          big(opportunity.gap).minus(opportunity.cost).gte(big(config.openOpportunity))
          && !openOpportunities.find(({ id }) => id === opportunity.id)
          && await validateOpportunityHistory(opportunity)
        ) {
          const gap = await waitForGapToWiden({ opportunity, getPrices })
          if (big(gap).minus(opportunity.cost).gte(big(config.openOpportunity))) {
            // OPEN
...
async function validateOpportunityHistory(opportunity) {
  const [
    exchangeLong,
    exchangeShort,
  ] = [
  ...
  const pairs = lodash.zip(candlesticksLong, candlesticksShort)
  for (const candlePair of pairs) {
    if (getGap(...candlePair.map(getClosingPrice), opportunity.investment) < config.closeOpportunity) {
      return true
    }
  }
...
const getGap = (bestBid, bestAsk, investment) => {
  const midPrice = big(bestBid).plus(bestAsk).div(2)
  const howMuchICanBuy = big(investment).times(2).div(midPrice)
  const difference = big(bestBid).minus(bestAsk)
  return howMuchICanBuy.times(difference)
}
...
function getUpdatedGap({ prices, opportunity }) {
  ...
  return getGap(bestBid.bid, bestAsk.ask, opportunity.investment)
}
...
function getOpportunity({ prices, ticket, investment }) {
  ...
  const gap = getGap(bestBid.bid, bestAsk.ask, investment)
  ...
  const opportunity = Object.seal({
    ...
    bestBid,
    gap: Number(gap),
    cost: Number(buyCost.plus(sellCost)),
    openedAt: null,
    closedAt: null,
  })
