...
async function validateOpportunityHistory({ opportunity, investment }) {
  ...
  const getGap = ([a, b]) => Math.abs(getClosingPrice(a) - getClosingPrice(b)) * investment
  const candlePairs = lodash.zip(candlesticksShort, candlesticksLong)
  let invalid = ''
  let previousGap = getGap(candlePairs.shift())

  const lowVolume = candlePairs.some(([short, long]) => getVolume(short) < 4 || getVolume(long) < 4)

  if (lowVolume) {
    invalid = 'low-volume'
  } else {
    const hasNotBeenClosable = !candlePairs.some(pair => getGap(pair).times(investment) < config.closeOpportunity)
    if (hasNotBeenClosable) {
      invalid = 'has-not-been-closable'
    }
  }
