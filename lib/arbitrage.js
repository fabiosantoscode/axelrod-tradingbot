  ...
  while (true) {
    ...
    for (const ticket of tickets) {
      try {
        ...
        if (
          opportunity.gain / investment >= config.openOpportunity
...
async function validateOpportunityHistory(opportunity) {
  ...
  const getVolume = candle => candle[5]
  const getClosingPrice = candle => candle[4]
  const getGap = ([a, b]) => Math.abs(getClosingPrice(a) - getClosingPrice(b))
  const candlePairs = lodash.zip(candlesticksShort, candlesticksLong)
  let valid = true
  let previousGap = getGap(candlePairs.shift())
  for (const candlePair of candlePairs) {
    const [shortCandle, longCandle] = candlePair
    if (
      getVolume(shortCandle) < 4 /* XXX: ensure market has dealt in at least 4 BTC so we don't stay in the dark */
      || getVolume(longCandle) < 4
    ) {
      valid = false;
      break;
    }
    const gap = getGap(candlePair)
    if (
      /* XXX: allow for the gap to become slightly larger */
      gap * 0.8 > previousGap
    ) {
      valid = false;
      break
