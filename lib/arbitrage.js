...
exports.closeOpportunitiesLoop = async ({ getPrices }) => {
  ...
  while (true) {
    ...
    for (const openOpp of openOpportunities) {
      const prices = await getPrices(openOpp.ticket)
      const opportunity = getOpportunity({ prices, ticket: openOpp.ticket, side: 'close' })
      const prevGap = previousGaps.get(openOpp) || NaN
      if (opportunity.gap !== prevGap) {
        previousGaps.set(openOpp, opportunity.gap)
        log.info({
          type: 'open-op-gap',
      ...
      if (opportunity.gap <= configs.closeOpportunity) {
        openOpportunities = openOpportunities
          .filter(({ id }) => id !== openOpp.id)
        writeOpportunitiesFile()
        opportunity.openedAt = openOpp.openedAt
...
function getOpportunity({ prices, ticket, side }) {
  if (side === 'open') {
  ...
  if (bestAsk.ask === 0 && side === 'open') {
    return
  }

  const oppId = ticket.symbol + '-' + bestAsk.exchangeName + '-' + bestBid.exchangeName

  // if side == buy and we return a bigger bestAsk than bestBid, we
  ...
  const gap = big(1).minus(big(bestBid.bid).div(bestAsk.ask))
